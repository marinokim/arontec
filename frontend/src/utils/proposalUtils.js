...
export const generateProposalExcel = async (proposalItems, user, setProposalItems, setShowProposalModal) => {
    ...
    worksheet.columns = [
        ...
        { header: '공급가(VAT+)', key: 'supplyPrice', width: 15 },
        { header: '개별배송비(VAT+)', key: 'shippingIndividual', width: 15 },
        { header: '카톤배송비(VAT+)', key: 'shippingCarton', width: 15 },
    ...
    for (let i = 0; i < proposalItems.length; i++) {
        ...
        row.values = {
            ...
            shippingIndividual: item.shipping_fee_individual ? parseInt(item.shipping_fee_individual) : 0,
            shippingCarton: item.shipping_fee_carton ? parseInt(item.shipping_fee_carton) : 0,
